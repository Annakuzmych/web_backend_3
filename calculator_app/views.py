...
def async_operations(request):
    return render(request, 'async_operations.html')

from .tasks import generate_pdf
from django.http import HttpResponse
from django.shortcuts import redirect
from django.urls import reverse
def save_history_pdf(request):
    if request.method == 'POST':
        # Отримання даних користувача
        user_id = request.user.id
        user_mail = request.user.email  # створити PDF для поточного користувача
        # Запуск таску Celery для створення PDF
        pdf_task = generate_pdf.delay(user_id,user_mail)
        # Повернення відповіді користувачеві, наприклад, перенаправлення на сторінку з історією користувача
        return redirect(reverse('history_view')) 
    else:
